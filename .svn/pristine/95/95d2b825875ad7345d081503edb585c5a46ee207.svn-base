...
Page({
  ...
  clickAddress(e) {
    ...
    qqmapsdk.reverseGeocoder({
      ...
      success(res) {
        ...
        wx.setStorageSync('LineName',data.Name);
        ...
        wx.setStorageSync('strAddress', data.StatingLocation_DetailAddress);
        wx.setStorageSync('strLatitude', data.StatingLocation_Latitude);
        wx.setStorageSync('strLongitude', data.StatingLocation_Longitude);
        wx.navigateBack({
          delta:1
        })
      }
  ...
  getXianluList(){
      ...
      http.getRequest('/Api/DispatchMobile/getPassengerLine?page='+page+'&limit=8&city=','', wx.getStorageSync('header'), res => {
        if(res.code==0){
          if(res.data.length){
            if(page == 1){
              that.setData({
